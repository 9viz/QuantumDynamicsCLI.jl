...
function dynamics(::QDSimUtilities.Method"unSMASH", units::QDSimUtilities.Units,
                  sys::QDSimUtilities.System, bath::QDSimUtilities.Bath,
                  sim::QDSimUtilities.Simulation, dt_group::Union{Nothing,HDF5.Group},
                  sim_node; dry=false)
    if !dry
        @info "Running a unSMASH calculation. Please cite:"
        QDSimUtilities.print_citation(unSMASH.references)
    end
    ...
    if !dry
        ...
        for n in 1:nbins
            ...
            @info "Calculating bin $n of $nbins"
            ρ = unSMASH.propagate(; Hamiltonian=Hamiltonian, Jw=bath.Jw,
                                  β=bath.β, num_osc=bath.num_osc,
                                  ρ0=ρ0, dt=sim.dt, ntimes=sim.nsteps,
                                  svec=bath.svecs, nmc=nmc, verbose=true,
                                  outgroup=outgroup, output=bin)
