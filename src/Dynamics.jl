...
function dynamics(::QDSimUtilities.Method"PLDM", units::QDSimUtilities.Units,
    ...
    solvers = Dict{String,Type{<:PLDM.Solver}}(
        "RK4" => PLDM.RK4,
        "Trotter" => PLDM.Trotter)
    samplers = Dict{String,Type{<:PLDM.PLDMSystemSampler}}(
        "Gaussian" => PLDM.Gaussian,
        "Rayleigh" => PLDM.Rayleigh)

    solver = get(sim_node, "solver", "RK4")
    sampler = get(sim_node, "sampler", "Rayleigh")
    sampler_group = Utilities.create_and_select_group(dt_group, "sampler=$sampler")
    solver_group = Utilities.create_and_select_group(sampler_group, "solver=$solver")
    data = Utilities.create_and_select_group(solver_group, "average")
    ...
    if !dry
        ...
        for n in 1:nbins
            ...
            @info "Calculating bin $n of $nbins"
            U0e, ρ = PLDM.propagate(; Hamiltonian=sys.Hamiltonian, Jw=bath.Jw,
                                    β=bath.β, num_osc=bath.num_osc,
                                    svec=bath.svecs, ρ0=ρ0, dt=sim.dt,
                                    ntimes=sim.nsteps, nmc=nmc,
                                    solver=solvers[solver], sampler=samplers[sampler],
                                    verbose=true, outgroup=outgroup,
                                    output=bin)
            ρs[n] = ρ
            U0es[n] = U0e
